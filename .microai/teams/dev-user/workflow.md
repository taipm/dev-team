...
# Templates (choose based on story type)
templates:
  user_story: "{installed_path}/templates/user-story-template.md"
  bug_fix: "{installed_path}/templates/bug-fix-template.md"
  technical_debt: "{installed_path}/templates/technical-debt-template.md"
  spike: "{installed_path}/templates/spike-template.md"
  meeting_minutes: "{installed_path}/templates/meeting-minutes-template.md"

# Knowledge (auto-loaded based on phase)
knowledge_index: "{installed_path}/knowledge/knowledge-index.yaml"
knowledge_path: "{installed_path}/knowledge/"

# Memory (persistent across sessions)
memory_path: "{installed_path}/memory/"
context_file: "{installed_path}/memory/context.md"
sessions_file: "{installed_path}/memory/sessions.md"
learnings_file: "{installed_path}/memory/learnings.md"
checkpoint_file: "{installed_path}/memory/checkpoint.yaml"
...
session:
  ...
  story_type: "user_story"           # user_story | bug_fix | technical_debt | spike

  # Dialogue Mode
  dialogue_mode: "manual"            # "manual" | "auto" | "semi-auto"
  # - manual: Wait for observer after every turn (default)
  # - auto: Agents dialogue automatically until story complete
  # - semi-auto: Auto until conflict/decision point, then ask observer

  auto_settings:
    max_auto_turns: 15               # Safety limit for auto mode
    pause_on_conflict: true          # Pause when agents disagree
    pause_on_scope_change: true      # Pause when scope changes
    pause_on_synthesis: true         # Pause before final story
    show_progress: true              # Display turns as they happen

  observer_mode: true                # User can intervene (always true)
  ...
  checkpoint_enabled: true           # Auto-save after each turn
...
| `*resume` | Resume từ last checkpoint |
| `*save` | Force save checkpoint ngay lập tức |
| `*type:<type>` | Change story type (user_story/bug_fix/technical_debt/spike) |
| `*auto` | Chuyển sang auto-dialogue mode (agents tự đối thoại) |
| `*manual` | Chuyển sang manual mode (chờ observer mỗi turn) |
| `*semi` | Chuyển sang semi-auto mode (auto nhưng pause tại decision points) |
...
on_session_start:
  - load: memory/context.md
    action: "Read current project context"
  - load: memory/sessions.md
    action: "Check for previous sessions on same topic"
  - load: memory/learnings.md
    action: "Apply learned patterns"
  - check: memory/checkpoint.yaml
    action: "Offer resume if checkpoint exists"
```
...
on_session_end:
  - update: memory/context.md
    with: "New project state, active topics"
  - append: memory/sessions.md
    with: "Session record"
  - update: memory/learnings.md
    with: "New patterns discovered"
  - archive: memory/checkpoint.yaml
    to: "memory/checkpoints/{date}-{subject}.yaml"
```
...
on_phase_change:
  requirements:
    load: knowledge/01-requirements-patterns.md
  clarification:
    load: knowledge/01-requirements-patterns.md
  negotiation:
    load:
      - knowledge/02-acceptance-criteria-guide.md
      - knowledge/03-conflict-resolution.md
  synthesis:
    load: knowledge/02-acceptance-criteria-guide.md

on_conflict_detected:
  load: knowledge/03-conflict-resolution.md
```
